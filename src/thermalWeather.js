...
export function calculateCloudbase(hum, press, temp, elev) {
  ...
  for (let i = elev; ; i += 1) {
    const pressureAtAlt = velitherm.pressureFromAltitude(
      i,
      seaLevelPress,
      temp
    );
    const tempAtAlt = temp - i * velitherm.gamma;
    const relHumAtAlt = velitherm.relativeHumidity(
      specHum,
      pressureAtAlt,
      tempAtAlt
    );
  ...
  const lclCB = velitherm.LCL(
    temp,
    velitherm.dewPoint(velitherm.relativeHumidity(specHum, press, temp), temp)
  );
...
function createThermalWeatherHTML(cB, lclCB) {
  return `
        ...
        <h3>The LCL cloudbase is <span>${Number.parseFloat(lclCB).toFixed(
          0
        )} meters.</span></h3>
    `;
}
...
function calculateSeaLevelPressure(temp, press, elev) {
  return (
    press * (1 - (0.0065 * elev) / (temp + 0.0065 * elev + 273.15)) ** -5.257
  );
}
